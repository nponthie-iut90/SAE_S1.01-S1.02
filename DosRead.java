...
public class DosRead {
    ...
    private static int byteArrayToInt(byte[] bytes, int offset, int fmt) {
        if (fmt == 16) return ((bytes[offset + 1] & 0xFF) << 8) | (bytes[offset] & 0xFF);
        else if (fmt == 32)
    ...
    public void readAudioDouble() {
        audio = new double[dataSize / 2]; // Assuming 16 bits per sample (2 bytes)
        ...
        try {
            ...
            for (int i = 0; i < dataSize / 2; i++) {
                int sample = byteArrayToInt(audioData, i * 2, 16);
                audio[i] = sample / 32768.0; // Normalisation des échantillons en valeurs de -1.0 à 1.0
            }
    ...
    public void audioRectifier() {
        if (audio != null) {
            ...
            System.out.println("Audio array is null. Please read audio data first.");
        }
    }
